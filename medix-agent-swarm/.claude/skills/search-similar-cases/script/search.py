...
async def search_similar_cases(query: str, max_results: int = 3) -> Dict[str, Any]:
    ...
    try:
        ...
        memory = get_memory_stack().long_term_memory
...
def format_cases(results: list) -> str:
    ...
    for i, case in enumerate(results, 1):
        ...
        pg_id = case.get("pg_memory_id", "")
        source = case.get("source", "mem0")
        output.append(f"【案例 {i}】（相似度: {score:.2%}，来源: {source}）")
        if pg_id:
            output.append(f"权威记录ID: {pg_id}")
