...
class SessionSummaryManager:
    ...
    负责保存和检索会话总结（PostgreSQL 权威 + Markdown 导出）
    """

    def __init__(
        self,
        base_dir: str = "memory/swarm/session_summaries",
        authoritative_store: Optional[Any] = None,
        user_id: Optional[str] = None,
    ):
        self.base_dir = Path(base_dir)
        self.base_dir.mkdir(parents=True, exist_ok=True)
        if authoritative_store is None:
            try:
                from .authoritative_store import get_authoritative_store
                authoritative_store = get_authoritative_store()
            except Exception:
                authoritative_store = None
        self.pg = authoritative_store
        try:
            from .settings import get_memory_config
            self.user_id = user_id or get_memory_config().get("default_user_id", "medix_user")
        except Exception:
            self.user_id = user_id or "medix_user"
    ...
    def save_summary(self, summary: SessionSummary):
        """保存会话总结：PG 账本 + Markdown 导出"""
        summary_path = self._get_summary_path(summary.session_id)

        try:
            content = summary.to_markdown()
            summary_path.write_text(content, encoding="utf-8")
            logger.info(f"Saved session summary markdown: {summary.session_id}")

            if self.pg and getattr(self.pg, "enabled", False):
                summary_json = {
                    ...
                    "agents_participated": [
                        {
                            "agent_id": a.agent_id,
                            "role": a.role,
                            "subtasks_handled": a.subtasks_handled,
                            "tool_calls": a.tool_calls,
                        }
                        for a in summary.agents_participated
                    ],
                    "subtasks_created": summary.subtasks_created,
                    "subtasks_completed": summary.subtasks_completed,
                    "final_answer_preview": summary.final_answer[:500],
                    "performance": {
                        "total_time": summary.performance.total_time,
                        "agent_count": summary.performance.agent_count,
                    },
                }
                self.pg.save_session_summary_row(
                    session_id=summary.session_id,
                    question=summary.question,
                    summary_json=summary_json,
                    markdown_path=str(summary_path),
