...
class ShortTermMemory:
    ...
    def __init__(
        ...
        Args:
            ...
            redis_config: 兼容旧版直连 Redis 配置
            redis_store: 推荐传入 memory.redis_store.RedisStore 单例
        """
        ...
        if storage_type == "redis":
            if redis_store is not None and redis_store.enabled:
                self.redis_client = redis_store.client
                logger.info("ShortTermMemory initialized with RedisStore")
            else:
                try:
                    import redis
                    ...
                    logger.info("ShortTermMemory initialized with Redis (legacy)")
    ...
    def _save_to_redis(self, history: ConversationHistory):
        ...
        if self.redis_store and self.redis_store.enabled:
            self.redis_store.save_session(history.session_id, payload)
            return
    ...
    def _load_from_redis(self, session_id: str) -> Optional[ConversationHistory]:
        """从 Redis 加载（内部方法）"""
        if self.redis_store and self.redis_store.enabled:
            data = self.redis_store.load_session(session_id)
            if data:
