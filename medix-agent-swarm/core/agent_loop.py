...
class AgentLoop:
                ...
                try:
                    ...
                    if llm_response.has_tool_calls():
                        ...
                        for tool_call in llm_response.tool_calls:
                            ...
                            tool_result = await self._execute_tool_with_cache(
                                agent,
                                tool_call.name,
                                tool_call.arguments,
                            )
    ...
    async def _execute_tool_with_cache(
        ...
        if self.tool_cache and getattr(self.tool_cache, "enabled", False):
            cached = self.tool_cache.get(tool_name, arguments)
            if cached is not None:
                return {**cached, "_from_cache": True}

        result = await agent.execute_tool(tool_name=tool_name, arguments=arguments)
