...
class SharedContext:
    ...
    def __init__(self, session_id: Optional[str] = None, redis_store: Optional[Any] = None):
        ...
        if redis_store and redis_store.enabled:
            loaded = redis_store.load_blackboard(self.session_id)
            if loaded:
                self._hydrate_from_snapshot(loaded)
                logger.debug(f"SharedContext hydrated from Redis: {self.session_id}")
    ...
    def to_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task_decomposition": {
                ...
                for tid, t in self.task_decomposition.items()
            },
            "agent_contributions": {
                aid: [
                    {
                ...
                for aid, contribs in self.agent_contributions.items()
            },
        }

    def _hydrate_from_snapshot(self, snap: Dict[str, Any]) -> None:
        self.data = snap.get("data", {})
        self.memory_pool = snap.get("memory_pool", {})
        self.events = []
        for ed in snap.get("events", []):
            self.events.append(Event(
                type=EventType(ed["type"]),
                source_agent=ed["source_agent"],
                data=ed.get("data", {}),
                id=ed.get("id", str(uuid.uuid4())),
                timestamp=datetime.fromisoformat(ed["timestamp"]),
                target_agents=ed.get("target_agents"),
            ))
        self.task_decomposition = {}
        for tid, td in snap.get("task_decomposition", {}).items():
            self.task_decomposition[tid] = SubTask(
                id=td["id"],
                type=td["type"],
                description=td["description"],
                assigned_agent=td["assigned_agent"],
        ...
        for aid, contribs in snap.get("agent_contributions", {}).items():
            for cd in contribs:
                self.agent_contributions[aid].append(Contribution(
                    agent_id=cd["agent_id"],
                    subtask_id=cd["subtask_id"],
                    result=cd["result"],
    ...
    def persist_to_redis(self) -> bool:
        if self._redis_store and self._redis_store.enabled:
            return self._redis_store.save_blackboard(self.session_id, self.to_snapshot())
        return False
