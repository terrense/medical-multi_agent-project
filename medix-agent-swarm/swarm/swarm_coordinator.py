...
from memory import SessionSummaryManager, SessionSummary, get_memory_stack
...
class SwarmCoordinator:
    ...
    def __init__(
        ...
        # 记忆栈：Redis + PostgreSQL + Mem0
        self._memory = get_memory_stack()
        self.session_manager = SessionSummaryManager(
            authoritative_store=self._memory.authoritative_store,
            user_id=self._memory.default_user_id,
        )
        self.short_term_memory = self._memory.short_term_memory
        self.long_term_memory = self._memory.long_term_memory
        self.tool_cache = self._memory.tool_cache
        self.redis_store = self._memory.redis_store

        for worker in self.worker_pool:
            if hasattr(worker, 'loop'):
                worker.loop.short_term_memory = self.short_term_memory
                worker.loop.tool_cache = self.tool_cache

        logger.info(f"SwarmCoordinator initialized with {len(self.worker_pool)} workers")
        logger.info(
            f"Memory: redis={self.redis_store.enabled}, "
            f"pg={self._memory.authoritative_store.enabled}, "
            f"mem0={self.long_term_memory.mem0_enabled}, "
            ...
            enhanced_context["historical_cases"] = [
                {
                    "summary": mem["content"],
                    "score": mem["score"],
                    "pg_memory_id": mem.get("pg_memory_id"),
                    "confidence": mem.get("confidence"),
                    "source": mem.get("source", "mem0"),
                }
    ...
    async def _process_with_swarm(
        ...
        shared_context = SharedContext(session_id=session_id, redis_store=self.redis_store)
        ...
        try:
            ...
            logger.info(f"Saved long-term memory PG-first (session={session_id})")

        except Exception as e:
            logger.error(f"Failed to save long-term memory: {e}")

        shared_context.persist_to_redis()
